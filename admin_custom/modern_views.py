...
@staff_member_required
def aurora_dashboard(request):
    """
    Aurora - intègre le frontend moderne (SPA) au sein de l'admin via un iframe.
    """
    # Accessible quel que soit le mode d'interface pour faciliter la bascule
    src = request.GET.get('src') or 'http://localhost:8080/'
    context = _get_modern_context(request, {
        'title': 'Aurora',
        'page': 'aurora',
        'frontend_url': src,
    ...
    return render(request, 'admin_custom/modern/aurora.html', context)
